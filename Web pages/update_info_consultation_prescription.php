<html>
 <body>
    <?php
      ...
      $ID_diagnostic_prescription = $_REQUEST['ID_diagnostic_prescription'];
      $name_UPDATE = $_REQUEST['Medication_Name'];
      $medication = explode(", ", $name_UPDATE);
      $dosage_UPDATE = $_REQUEST['dosage'];
      $description_UPDATE = $_REQUEST['description'];

      $result = $stat->execute([$medication[0], $medication[1],$VAT_doctor_UPDATE, $Date_UPDATE, $ID_diagnostic_prescription,
    $dosage_UPDATE, $description_UPDATE]);
      if ($result == FALSE)
      {
